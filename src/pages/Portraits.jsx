...
const Portraits = () => {
  ...
  useEffect(() => {
    ...
    const query = photographer
      ? `*[_type == "gallery" && category == "portraits" && photographer == $photographer] | order(_createdAt desc) {
          _id, title, slug, date,
          "imageCount": count(images),
          "coverImage": coalesce(coverImage, images[0]),
          "lqip": coalesce(coverImage.asset->metadata.lqip, images[0].asset->metadata.lqip)
        }`
      : `*[_type == "gallery" && category == "portraits"] | order(_createdAt desc) {
          _id, title, slug, date,
          "imageCount": count(images),
          "coverImage": coalesce(coverImage, images[0]),
          "lqip": coalesce(coverImage.asset->metadata.lqip, images[0].asset->metadata.lqip)
        ...
        <div className="layout" style={{ paddingTop: '150px', paddingBottom: '100px', position: 'relative', zIndex: 1, flex: 1 }}>
            ...
            >
                <h3 style={{ fontSize: '0.9rem', letterSpacing: '0.3em', color: '#aaa', marginBottom: '1.5rem', fontFamily: "'Inter', sans-serif", textTransform: 'uppercase' }}>
                    {photographer ? `${photographer.charAt(0).toUpperCase() + photographer.slice(1)} — ` : ''}Editorial & Personal
                </h3>
                <h1 style={{ 
            ...
            >
              {items.length === 0 ? (
                ...
                >
                  ...
                  <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '0.8rem', color: '#555', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '2rem' }}>Add portrait galleries in Sanity Studio to display them here.</p>
